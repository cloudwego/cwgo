...
func (s *GenerateCodeService) Run(req *agent.GenerateCodeReq) (resp *agent.GenerateCodeRes, err error) {
	...
	switch repo.Type {
	case consts.GitLab:
		err := RunGitlab(s, repo.RepositoryUrl, idl.MainIdlPath, idl.ServiceName)
		if err != nil {
			resp.Code = 400
			resp.Msg = err.Error()
			return resp, err
	...
	case consts.Github:
		err := RunGithub(s, repo.RepositoryUrl, idl.MainIdlPath, idl.ServiceName)
		if err != nil {
			resp.Code = 400
			resp.Msg = err.Error()
			return resp, err
...
func RunGitlab(s *GenerateCodeService, repoPath, idlPath, serviceName string) error {
	...
	ref := consts.MainRef
	file, err := s.svcCtx.RepoManager.Gitlab.GetFile(owner, repoName, idlPid, ref)
	if err != nil {
		return err
	}
	...
	err = s.svcCtx.RepoManager.Gitlab.PushFilesToRepository(fileContentMap, owner, repoPath, ref, "generated by cwgo")
	if err != nil {
		return err
	}
...
func RunGithub(s *GenerateCodeService, repoPath, idlPath, serviceName string) error {
